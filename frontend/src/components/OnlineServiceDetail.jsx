...
export default function OnlineServiceDetail() {
  const { id } = useParams();  
  ...
  useEffect(() => {
    const fetchOnlineService = async () => {
      try {
        ...
        setService(response.data); // Set the expert data from the API response
      } catch (error) {
  ...
  console.log("online service"  , service);
  ...
  navigate(`/online-services/${service._id}/booking`);
};
