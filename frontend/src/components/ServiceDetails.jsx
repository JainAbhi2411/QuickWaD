...
export default function ServiceDetails() {
  ...
  const handleBookNow = () => {
    // Extract the service type from the service data (if available in the response)
    let serviceType = service?.type || ''; // You need to have the `type` field in the backend
    ...
    // Adjust the booking path based on service type
    let bookingPath = '';
    switch (serviceType) {
      case 'online':
        bookingPath = `/online-services/${service._id}/booking`;
        break;
      case 'premium':
        bookingPath = `/premium-services/${service._id}/booking`;
        break;
      case 'expert':
        bookingPath = `/book-expert/${service._id}`;
        break;
      case 'category':
        bookingPath = `/service/${service._id}/booking`;
        break;
      default:
        bookingPath = `/service/${service._id}/booking`; // Fallback to default
        break;
    }
  ...
  if (loading) return <div style={{ padding: '40px' }}>Loading service details...</div>;
